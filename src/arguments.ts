...
export const moveArgument = async (direction: 'left' | 'right') => {
    ...
    const args = smartSplitPreserveWhitespace(argsText);
    ...
    const argIndex = findArgumentIndex(args, cursorOffsetInArgs);
    ...
    for (let i = 0; i < targetIndex; i++) {
        newCursorOffsetInArgs += newArgs[i].length + 2; // +2 for ", "
    }
