...
suite('Add Numbers To Cursors Test Suite', () => {
    ...
    test('addNumbersToCursorsPro test', async () => {
        const doc = await vscode.workspace.openTextDocument({
            content: 'item\nitem\nitem'
        });
        const editor = await vscode.window.showTextDocument(doc);
        ...
        await myExtension.addNumbersToCursorsPro(42, 2);

        const text = doc.getText();
        const lines = text.split('\n');
        assert.strictEqual(lines[0], 'item42');
        assert.strictEqual(lines[1], 'item44');
        assert.strictEqual(lines[2], 'item46');
    });
});
